package com.sdl.selenium.bootstrap.window;

import com.sdl.selenium.bootstrap.button.Button;
import com.sdl.selenium.conditions.ConditionManager;
import com.sdl.selenium.conditions.ElementRemovedSuccessCondition;
import com.sdl.selenium.web.WebLocator;
import com.sdl.selenium.web.utils.internationalization.InternationalizationUtils;

public class PopOver extends WebLocator {

    private PopOver() {
        withInfoMessage("PopOver");
        withClasses("popover");
        withTemplate("title", "count(.//*[@class='popover-title' and text()='%s'])> 0");
    }

    public PopOver(String title, String message) {
        this(title);
        withTemplate("title", "count(.//*[@class='popover-title' and text()='%s']//following-sibling::*[@class='popover-content' and text()='" + message + "'])> 0");
    }

    public PopOver(String title, String message, boolean isInternationalized) {
        this(title, isInternationalized);
        message = InternationalizationUtils.getInternationalizedText(message, isInternationalized);
        withTemplate("title", "count(.//*[@class='popover-title' and text()='%s']//following-sibling::*[@class='popover-content' and text()='" + message + "'])> 0");
    }

    public PopOver(String title) {
        this();
        withTitle(title);
    }

    public PopOver(String title, boolean isInternationalized) {
        this();
        withTitle(title, isInternationalized);
    }

    public boolean close() {
        return new Button(this, "Close").click() && waitClose(this, 2000);
    }

    private boolean waitClose(WebLocator popOver, long time) {
        return new ConditionManager(time).add(new ElementRemovedSuccessCondition(popOver)).execute().isSuccess();
    }
}