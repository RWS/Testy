...
//        setTemplate("groupTextRow", "count(*//text()[contains(.,'%s')]) > 0");
    ...
    public List<Row> getRows() {
        ...
        if (Strings.isNullOrEmpty(toGroup)) {
            ...
            group = new WebLocator(grid).setTag("").setRoot("//table[//table[count(.//*[@class='x-grid-group-title' and contains(.,'" + nameGroup + "')]) > 0][1] | preceding-sibling::table//*[1][count(.//*[@class='x-grid-group-title' and contains(.,'" + nameGroup + "')]) > 0] and following-sibling::table[count(.//*[@class='x-grid-group-title' and contains(.,'" + toGroup + "')]) > 0]]");
        }
        int size = group.size();
