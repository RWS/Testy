...
public class Group extends AbstractCell {
...
//        setTemplate("groupTextRow", "count(*//text()[contains(.,'%s')]) > 0");
    ...
    public Group(Locator container) {
        this();
        setContainer(container);
    }

    public Group(Locator container, int indexRow) {
        this(container);
        setPosition(indexRow);
    }

    public Group(Locator container, String groupName) {
        this(container);
        nameGroup = groupName;
        group.setTemplateValue("title", groupName);
    }

    public Group(Locator container, String groupName, int indexRow) {
    ...
    public List<Row> getRows() {
        Grid grid = (Grid) getXPathBuilder().getContainer();
