...
public class RowTest {
    ...
    private static Table table = new Table().withId("ID");
    private static Cell tableCell = new Cell(3, "1234", SearchType.EQUALS).withTag("th");

    @DataProvider
    public static Object[][] testConstructorPathDataProvider() {
        return new Object[][]{
                {new Row(),      "//tr"},
                {new Row().withVisibility(true),      "//tr[count(ancestor-or-self::*[contains(@style, 'display: none')]) = 0]"},
                ...
                {new Row(table, new Cell(3, "1234", SearchType.EQUALS), new Cell(4, "Eng-Fra", SearchType.EQUALS)).withVisibility(true), "//table[@id='ID']//tr[count(.//td[3][(.='1234' or count(*//text()[.='1234']) > 0)]) > 0 and count(.//td[4][(.='Eng-Fra' or count(*//text()[.='Eng-Fra']) > 0)]) > 0 and count(ancestor-or-self::*[contains(@style, 'display: none')]) = 0]"},
