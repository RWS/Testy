...
public class TextsTest extends WebLocator {
    private final SearchType C1 = SearchType.EQUALS;
    private final SearchType C2 = SearchType.TRIM;

    private final WebLocator default_el = new WebLocator().setBaseCls("btn").setText("Cancel", C1, C2);
    private final WebLocator deep_child = new WebLocator().setBaseCls("btn").setText("Cancel", SearchType.DEEP_CHILD_NODE, C1, C2);
    private final WebLocator deep_child_self = new WebLocator().setBaseCls("btn").setText("Cancel", SearchType.DEEP_CHILD_NODE_OR_SELF, C1, C2);
    private final WebLocator child_node = new WebLocator().setBaseCls("btn").setText("Cancel", SearchType.CHILD_NODE, C1, C2);
    private final WebLocator containsAllChildNodes = new WebLocator().setBaseCls("btn").setText("|This's|was|good", SearchType.CONTAINS_ALL_CHILD_NODES);
    private final WebLocator containsAll = new WebLocator().setBaseCls("btn").setText("|This's|was|good", SearchType.CONTAINS_ALL);
    private final WebLocator containsAny = new WebLocator().setBaseCls("btn").setText("|This's|was|good", SearchType.CONTAINS_ANY);
