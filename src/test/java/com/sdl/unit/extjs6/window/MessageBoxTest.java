...
public class MessageBoxTest {
    ...
    public static Object[][] testConstructorPathDataProvider() {
        return new Object[][]{
                ...
                {new MessageBox("Title", "Message", SearchType.DEEP_CHILD_NODE_OR_SELF),  "//*[contains(concat(' ', @class, ' '), ' x-message-box ') and not(contains(@class, 'x-hidden-offsets')) and count(.//*[contains(concat(' ', @class, ' '), ' x-header ')]//*[normalize-space(text())='Title']) > 0 and count(//*[contains(concat(' ', @class, ' '), ' x-window-body ') and not(contains(@class, 'x-window-body-fullscreen'))]//*[(contains(.,'Message') or count(*//text()[contains(.,'Message')]) > 0)]) > 0]"},
                {new MessageBox("Title", "Message"), "//*[contains(concat(' ', @class, ' '), ' x-message-box ') and not(contains(@class, 'x-hidden-offsets')) and count(.//*[contains(concat(' ', @class, ' '), ' x-header ')]//*[normalize-space(text())='Title']) > 0 and count(//*[contains(concat(' ', @class, ' '), ' x-window-body ') and not(contains(@class, 'x-window-body-fullscreen'))]//*[contains(text(),'Message')]) > 0]"},
        };
    }
